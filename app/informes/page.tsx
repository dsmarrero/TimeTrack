...
export default async function InformesPage({
  ...
  const mobileLabelClass =
    "text-xs font-semibold uppercase tracking-wide text-foreground/40 md:hidden";
      ...
      <div className="mx-auto max-w-3xl p-8">
        ...
        <div className="mt-6 rounded-xl border border-border p-6">
          ...
          <ul className="mt-3 divide-y divide-border text-sm">
            ...
            {byProject.size === 0 && (
              <li className="py-2 text-foreground/50">Sin datos</li>
            )}
          </ul>

          <ProjectTimeChart
            data={[...byProject.entries()].map(([id, row]) => ({
              id,
              name: row.name,
              minutes: row.minutes,
            }))}
        ...
        <div className="mt-6 md:overflow-x-auto md:rounded-xl md:border md:border-border">
          ...
          {crossTable.size === 0 && (
            <p className="mt-2 px-1 text-sm text-foreground/50 md:px-4 md:py-4">
