...
type ActiveEntryWithProject = Prisma.TimeEntryGetPayload<{
  include: { project: true };
}>;

type TodayEntryWithProject = Prisma.TimeEntryGetPayload<{
  include: { project: true };
}>;

type AdminActiveEntry = Prisma.TimeEntryGetPayload<{
...
export default async function DashboardPage() {
  ...
  const [activeEntryRaw, projects, todayEntries, othersActiveEntries] = await Promise.all([
    ...
    prisma.timeEntry.findMany({
      ...
      include: { project: true } 
    }) as Promise<TodayEntryWithProject[]>,
isAdmin
      ? (prisma.timeEntry.findMany({
          where: { endedAt: null, employeeId: { not: employee.id } },
          include: { employee: true, project: true },
          orderBy: { startedAt: "asc" },
        }) as Promise<AdminActiveEntry[]>)
      : Promise.resolve([]),
  ]);
...
const activeEntry = activeEntryRaw
    ? {
        ...activeEntryRaw,
        startedAt: activeEntryRaw.startedAt.toISOString(),
        endedAt: activeEntryRaw.endedAt ? activeEntryRaw.endedAt.toISOString() : null,
      }
    : null;
